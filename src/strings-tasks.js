...
function removeFirstOccurrences(str, value) {
  ...
  // const searchValue = str.indexOf(value);
  // if (searchValue !== -1) {
  //   const partBefore = str.substring(0, searchValue);
  //   const partAfter = str.substring(searchValue + value.length);
...
function removeLastOccurrences(str, value) {
  const searchValue = str.lastIndexOf(value);
  if (searchValue !== -1) {
    const firstPart = str.substring(0, searchValue);
    const secondPart = str.substring(searchValue + value.length);
...
function countVowels(str) {
  ...
  // const vowelStr = Array.from(str.matchAll(new RegExp(`[${vowels}]`, 'g')));
...
function isPalindrome(str) {
  const strCleaned = str.replace(/[^a-zA-Z]/g, '').toLocaleLowerCase();
  const reversedStr = strCleaned.split('').reverse().join('');
  if (reversedStr === strCleaned) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  sentenceArr.sort((a, b) => b.length - a.length);
...
function extractNameFromTemplate(value) {
  const partBefore = value.indexOf(',');
  let result2;
  if (partBefore !== -1) {
    const result1 = value.slice(partBefore + 2); // two spaces after coma //
    result2 = result1.slice(0, result1.length - 1);
  }
  return result2;
}
